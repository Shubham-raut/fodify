...
function MyRoute() {
  ...
  const toPayment = (cartData) => {
    let { cartItems, uid } = cartData;
    setCartData(cartItems);
    history.push(`/payment${uid}`);
  ...
  return (
    <div>
      ...
      <Switch>
        <Route path="/payment:firebaseUid" component={() => <Payment cartData={cartData} />} />
      </Switch>
      <Switch>
      ...
      <Switch>
        <Route
          path="/restaurent:id"
          component={() => <RestaurentDetails res={res} toPayment={toPayment} />}
        />
      </Switch>
    </div>
  );
